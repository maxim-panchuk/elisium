...
def extract_words(text: str) -> list[str]:
    """
    Removes all punctuation marks from the string using a regex,
    then splits it by spaces.
    Returns a list of words.
    """
    text_no_punct = re.sub(r'[^\w\sа-яА-ЯёЁ]', '', text, flags=re.UNICODE)
    return text_no_punct.split()

def transcribe_audio(path_to_audio: str) -> str | None:
    """
    Launches echogarden via CLI to perform force-alignment.
    Ensures the result is saved to RESULT_JSON_FILE,
    overwriting any existing file of the same name.
    Returns RESULT_JSON_FILE on success; otherwise None.
    """
    # If a previous result file exists, remove it to force overwrite
    if os.path.exists(RESULT_JSON_FILE):
        os.remove(RESULT_JSON_FILE)

    cmd = ['echogarden', 'transcribe', path_to_audio, RESULT_JSON_FILE]

    result = subprocess.run(cmd, capture_output=True, text=True)
    if result.returncode == 0:
        print(f"Force-alignment result is in {RESULT_JSON_FILE}")
        return RESULT_JSON_FILE
    else:
        print(f"Force-alignment error (exit code {result.returncode}) while running echogarden")
        print("STDOUT:", result.stdout)
        print("STDERR:", result.stderr)
        return None


def parse_echogarden_json(file_path: str) -> list[dict]:
    """
    Reads the JSON file generated by echogarden and extracts
    a list of words with start/end timestamps.
...
def make_subtitles(text: str, path_to_audio: str) -> list[TextClip]:
    """
